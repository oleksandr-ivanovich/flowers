...
export default function AddSalePage() {
  ...
  const customerSearch = useQuery({
    ...
    enabled: customer === null && !showNewCustomer,
  });

  const createCustomer = useMutation({
    mutationFn: () =>
      apiPost<Customer>("/api/customers", { name: newName.trim(), phone: newPhone.trim() }),
    onSuccess: (c) => {
      setCustomer(c);
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 rounded-lg bg-gray-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-40"
              >
                {createCustomer.isPending ? "Зберігаємо…" : "Зберегти клієнта"}
              </button>
              <button
                type="button"
                onClick={() => {
          ...
          <>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="rounded-lg border px-3 py-2 text-sm hover:bg-gray-100 whitespace-nowrap"
              >
                + Новий
              </button>
            </div>
